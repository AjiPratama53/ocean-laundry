  ...
  <v-container class="h-full flex items-center" max-width="900">
    <div>
      ...
      <div class="grid md:grid-cols-2 gap-4">
        <v-card class="hero-card md:col-span-2" image="https://cdn.vuetifyjs.com/docs/images/one/create/feature.png"
          rounded="3xl" variant="flat">
          <template #prepend>
        ...
        <v-card v-for="link in links" :href="link.href" :key="link.href" :subtitle="link.subtitle" :title="link.title"
          class="
            h-full py-3 rounded-3xl transition-[border-radius] hover:rounded-lg
            flex items-center [&>.v-card-item]:w-full
            group" rel="noopener noreferrer" target="_blank" variant="flat">
          <template #prepend>
            <v-avatar :icon="link.icon" size="60" variant="tonal" class="ml-2 mr-4" />
          </template>
          <template #append>
            <v-icon class="ml-1 opacity-0 transition group-hover:opacity-90 group-hover:-translate-x-1"
