...
function env(name: string): string | undefined {
  ...
  try {
    const vite = (import.meta as unknown as { env?: Record<string, string | undefined> }).env?.[name]
    if (vite !== undefined) return vite
  }
  catch { /* import.meta.env unavailable outside Vite */ }
  try {
    const node = (globalThis as unknown as { process?: { env?: Record<string, string | undefined> } }).process?.env
    return node?.[name]
  }
  catch {
    return undefined
...
function tokenUrl(): string {
  ...
  if (!v) throw new AuthError('unreachable', 'Login server address (VITE_OIDC_TOKEN_URL) is not configured.')
  return v
}
...
async function tokenRequest(params: Record<string, string>): Promise<TokenResponse> {
  ...
  try {
    ...
    throw new AuthError('unreachable', err instanceof Error ? `Cannot reach the login server: ${err.message}` : 'Cannot reach the login server.')
  }
  const body = (await res.json().catch(() => ({}))) as { error?: string; error_description?: string } & Partial<TokenResponse>
  if (!res.ok || !body.access_token) {
    // Keycloak answers wrong credentials / dead refresh with invalid_grant.
    throw new AuthError(body.error ?? `http-${res.status}`, body.error_description ?? 'Sign-in failed.')
  }
...
export async function passwordGrant(username: string, password: string): Promise<TokenResponse> {
  return tokenRequest({ grant_type: 'password', username, password, scope: `openid ${DOMAIN_SCOPES}` })
}
...
function silentRefresh(): Promise<string | null> {
  if (!refreshPromise) {
    ...
    refreshPromise = tokenRequest({ grant_type: 'refresh_token', refresh_token: rt })
      .then((t) => {
...
async function attempt<T>(url: string, key: string, path: string, opts: RequestOptions, token: string | null, stored: string | null | undefined): Promise<Response> {
  ...
  try {
    ...
    throw new ApiError(0, fallbackProblem(0, path, err instanceof Error ? err.message : 'Network request failed'), true)
  }
}

async function requestRaw<T>(path: string, opts: RequestOptions = {}): Promise<ReadResult<T>> {
  ...
  if (res.status === 304) {
    return { data: null, etag: stored ?? etag, notModified: true, fetchedAt: new Date() }
  }
...
async function readProblem(res: Response, path: string): Promise<Problem> {
  ...
  if (body && typeof body === 'object' && ('title' in body || 'detail' in body)) {
    ...
    return {
      ...
      title: typeof b.title === 'string' ? b.title : `Request failed (${res.status})`,
      status: typeof b.status === 'number' ? b.status : res.status,
      detail: typeof b.detail === 'string' ? b.detail : 'The service refused the request.',
      instance: typeof b.instance === 'string' ? b.instance : path,
      ...(Array.isArray(b['invalid-params']) ? { 'invalid-params': b['invalid-params'] as Problem['invalid-params'] } : {}),
    }
  }
  return fallbackProblem(res.status, path, typeof body === 'string' && body ? body : `The service answered ${res.status}.`)
}

/** Convenience for non-conditional calls (forms, actions). */
async function call<T>(path: string, opts: RequestOptions = {}): Promise<{ data: T; etag: string | null }> {
...
export type OrderStatus
  = | 'placed' | 'picked_up' | 'weighed' | 'awaiting_payment'
    | 'washing' | 'ready' | 'delivering' | 'completed' | 'cancelled'
...
export function getPackageConditional(id: string, etag?: string | null) {
  return requestRaw<Package>(`/packages/${encodeURIComponent(id)}`, { conditional: true, etag })
}

export function createPackage(input: { packageName: string; packageDesc: string; packagePrice: number }) {
  return call<Package>('/packages', { method: 'POST', body: input })
}

export function updatePackage(id: string, input: { packageName?: string; packageDesc?: string; packagePrice?: number }, etag?: string | null) {
  return call<Package>(`/packages/${encodeURIComponent(id)}`, {
    method: 'PATCH', body: input, withPrecondition: true, etag: etag ?? undefined,
  })
}

export function deletePackage(id: string, etag?: string | null) {
  return call<Package>(`/packages/${encodeURIComponent(id)}`, {
    method: 'DELETE', withPrecondition: true, etag: etag ?? undefined,
  })
}
...
export function getOrderConditional(id: string, etag?: string | null) {
  return requestRaw<Order>(`/orders/${encodeURIComponent(id)}`, { conditional: true, etag })
}
...
export function createOrder(
  ...
  return call<Order>('/orders', { method: 'POST', body: input, idempotencyKey })
}

function transition(id: string, segment: string, body?: unknown, etag?: string | null) {
  return call<Order>(`/orders/${encodeURIComponent(id)}/${segment}`, {
    method: 'POST', body, withPrecondition: true, etag: etag ?? undefined,
  })
}

export const pickupOrder = (id: string, etag?: string | null) => transition(id, 'pickup', {}, etag)
export const weighOrder = (id: string, weightGrams: number, etag?: string | null) =>
  transition(id, 'weigh', { weightGrams }, etag)
export const washOrder = (id: string, etag?: string | null) => transition(id, 'wash', {}, etag)
export const readyOrder = (id: string, etag?: string | null) => transition(id, 'ready', {}, etag)
export const deliverOrder = (id: string, etag?: string | null) => transition(id, 'delivery', {}, etag)
export const completeOrder = (id: string, etag?: string | null) => transition(id, 'complete', {}, etag)
export const cancelOrder = (id: string, etag?: string | null) => transition(id, 'cancel', {}, etag)
...
export function createPayment(
  ...
  return call<Payment>('/payments', { method: 'POST', body: input, idempotencyKey })
}

export function getPaymentConditional(id: string, etag?: string | null) {
  return requestRaw<Payment>(`/payments/${encodeURIComponent(id)}`, { conditional: true, etag })
}
